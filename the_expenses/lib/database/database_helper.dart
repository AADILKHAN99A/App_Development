...
class DatabaseHelper {
  ...
  Future _onCreate(Database db, int version) async {
    await db.execute(''' CREATE TABLE $userTable(
        ${UserTableFields.columnID} INTEGER PRIMARY KEY,
        ${UserTableFields.columnUserId} TEXT NOT NULL,
        ${UserTableFields.columnEmail} TEXT NOT NULL
        )''');
  ...
  Future insertUser({required userid, required email}) async {
    ...
    const columns =
        '${UserTableFields.columnUserId},${UserTableFields.columnEmail}';

    final id = await db!.rawInsert(
        'INSERT INTO $userTable($columns)VALUES(?,?)', ['$userid', '$email']);
